...
class PredictionSingleGP(Prediction):
    def __init__(self, context):
        super(PredictionSingleGP, self).__init__()
    ...
    def build_variance_standard(self):
        ...
        for l in range(self.num_components):
            mu_f, sigma_f, _, _ = self.sparsity._build_intermediate_conditionals(l, self.a, self.x_test, predict=not full_var_flag)
        ...
        for k in range(self.num_components):
            #mu_f = [Q, N, 1]
            #mu_w = [Q, P, N, 1]
            mu_f, sigma_f, _, _ = self.sparsity._build_intermediate_conditionals(k, self.a, self.x_test, predict=not full_var_flag)
            mu_f, sigma_f = self.get_expected_values(mu_f, sigma_f)
            ...
            s = tf.Print(s, [tf.reduce_sum(sigma_f)], 'tf.reduce_sum(sigma_f): ')
            s += noise_sigma
            ...
            for l in range(self.num_components):
                ...
                mu_f_l = precomp_intermediate[l][1][j,:,0]
