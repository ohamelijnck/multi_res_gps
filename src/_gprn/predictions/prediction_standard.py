...
class PredictionStandard(Prediction):
    def __init__(self, context):
        super(PredictionStandard, self).__init__()
    ...
    def build_variance_standard(self):
        ...
        for l in range(self.num_components):
            mu_f, sigma_f, mu_w, sigma_w = self.sparsity._build_intermediate_conditionals(l, self.a, self.x_test, predict=full_var_flag)
        ...
        for k in range(self.num_components):
            #mu_f = [Q, N, 1]
            #mu_w = [Q, P, N, 1]
            mu_f, sigma_f, mu_w, sigma_w = self.sparsity._build_intermediate_conditionals(k, self.a, self.x_test, predict=full_var_flag)
            #mu_f, sigma_f, mu_w, sigma_w = self.get_expected_values(mu_f, sigma_f, mu_w, sigma_w)
            ...
            for i in range(self.num_outputs):
                ...
                noise_sigma = tf.Print(noise_sigma, [noise_sigma, tf.square(util.var_postive(self.sigma_y[i]))], 'noise_sigma: ')

                mu_wi = mu_w[i,:,:,:] # Q x N x 1
                sigma_wik = sigma_w[i,:,:] # Q x N 

                s = tf.reduce_sum(tf.multiply(sigma_f, sigma_wik), axis=0)
                s += noise_sigma
                s += tf.reduce_sum(tf.multiply(tf.multiply(mu_f[:,:,0], mu_f[:,:,0]), sigma_wik), axis=0)
                s += tf.reduce_sum(tf.multiply(tf.multiply(mu_wi[:, :, 0], mu_wi[:, :, 0]), sigma_f), axis=0)
        ...
        for k in range(self.num_components):
            ...
            for i in range(self.num_outputs):
                for j in range(self.num_latent):
                    ...
                    w_kij_arr = mu_wij
                    #wf = tf.matmul(w_kij_arr, mu_fj)
                    wf = tf.multiply(w_kij_arr, mu_fj)
                    #wf = mu_fj
                    #wf = w_kij_arr
