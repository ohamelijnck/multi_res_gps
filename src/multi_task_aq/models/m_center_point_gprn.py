...
def get_config():
    return  [
        {
            'name': 'center point gprn',
            #'model_name': 'mr_gprn',
            'file_prefix': 'center_point_gprn',
            'experiment_id': 0,
            'num_outputs': 1,
...
def get_context(CONFIG, X, Y, test, r):
    ...
    context.kernels = [
        {
            ...
            'f': [ gprn.kernels.MR_SE(num_dimensions=D, length_scale=inv(0.1)) for i in range(context.num_latent) ],
            ...
            'w': [[gprn.kernels.MR_SE(num_dimensions=D, length_scale=inv(3.0)) for j in range(context.num_latent)] for i in range(context.num_outputs)]
...
def run(CONFIG, test=None, r=1, return_m=False, force_restore=False):
   ...
   # Y[0] = np.concatenate([np.empty(Y[0].shape), Y[0]], axis=1)
    ...
    context =  get_context(CONFIG, X, Y, test_id, r)
