...
def prediction(CONFIG, m, test_id, r, X, XS, XS_VIS):
    ...
    X_mu, X_sig = batch_predict(m, X, num_samples)
    ...
    if True:
        ys_fine_grid_0, ys_var_fine_grid_0 = batch_predict(m, XS_VIS, num_samples)
        ...
        np.save('../results/{prefix}_y_vis_{test_id}_{r}'.format(prefix=CONFIG['file_prefix'], r=r, test_id=test_id), pred_ys_fine_grid)
...
def run(CONFIG, test=None, r=1, return_m=False, force_restore=False):
    ...
    #X[0],Y[0] is the AGGR data
    #X[1],Y[1] is the point data
    ...
    def make_mixture(dataset, parent_mixtures = None, name_prefix=''):
        base_kernel_ad = range(dataset[0][0].shape[-1])
        base_kernel_ls = [0.01 for i in base_kernel_ad]
        ...
        base_Z = [get_inducing_points(dataset[0][0], None), get_inducing_points(dataset[1][0], None)]

        sliced_dataset = dataset[0][0][:, 0, :]

        dgp_Z = get_inducing_points(dataset[0][1], 100)
        ...
        noise_sigmas = [[0.1, 0.1], [0.1], [1.0]]
        minibatch_sizes = [50, 50]
    ...
    if restore:
        ...
        saver.restore(tf_session, DATA_ROOT+'restore/{name}_{test}_{r}.ckpt'.format(name=CONFIG['file_prefix'], test=test_id, r=r))
    ...
    if train:
        ...
        print('saving')
        saver = tf.train.Saver()
        save_path = saver.save(tf_session, 'restore/{name}_{test}_{r}.ckpt'.format(name=CONFIG['file_prefix'], test=test_id, r=r))
    ...
    prediction(CONFIG, m, test_id, r, rs(X[1]), rs(XS), rs(XS_VIS))
